...
/// # Safety
///
/// Every pointer in `args` must satisfy the same validity, length, and string
/// termination requirements as [`generate_user_config`]. `period_secs`, when
/// non-null, must point to an aligned, initialized `u64` readable for this
/// call.
#[must_use]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn generate_user_config_with_bootstrap_period(
    ...
    let period = (!period_secs.is_null()).then(|| {
        // SAFETY: the caller guarantees a readable, aligned u64 when non-null.
        unsafe { *period_secs }
    });
