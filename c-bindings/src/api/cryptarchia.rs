...
mod tests {
    ...
    fn cryptarchia_info_abi_version_matches_the_current_layout() {
        ...
        assert_eq!(CRYPTARCHIA_INFO_ABI_VERSION, 2);
        assert_eq!(std::mem::offset_of!(CryptarchiaInfo, lib), 0);
        assert_eq!(std::mem::offset_of!(CryptarchiaInfo, tip), 32);
        assert_eq!(std::mem::offset_of!(CryptarchiaInfo, slot), 64);
        assert_eq!(std::mem::offset_of!(CryptarchiaInfo, height), 72);
        assert_eq!(std::mem::offset_of!(CryptarchiaInfo, mode), 80);
        assert_eq!(std::mem::offset_of!(CryptarchiaInfo, genesis_id), 84);
    }
