...
impl Utxo {
    ...
    // The id doesn't depend on the channel, so it is preserved.
    #[must_use]
    pub const fn to_channel(self, channel_id: ChannelId) -> Self {
        Self::Channel {
            utxo_data: *self.data(),
            channel_id,
        }
    ...
    #[must_use]
    pub const fn to_bedrock(self) -> Self {
        Self::Bedrock(*self.data())
    }
}
