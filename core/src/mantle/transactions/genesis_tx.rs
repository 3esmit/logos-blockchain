...
/// Decode deployment inscriptions in both the current compact format and the
/// historical published format used by existing networks.
fn decode_cryptarchia_parameter(
    input: &[u8],
) -> Result<(&[u8], CryptarchiaParameter), DecodeError> {
    // Prefer a complete compact encoding. This avoids interpreting a valid
    // chain ID containing NUL bytes as a legacy length prefix.
    ...
    // Published profiles encoded both the chain-ID length and timestamp as
    // u64 and reduced the epoch nonce into Fr. Retain that wire compatibility
...
mod tests {
    ...
    fn published_testnet_legacy_deployment_is_accepted() {
        ...
        .expect("published deployment inscription must be valid hex");

        let (rest, decoded) = decode_cryptarchia_parameter(&encoded)
            .expect("published legacy deployment must decode");
