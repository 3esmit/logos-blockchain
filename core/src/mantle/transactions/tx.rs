...
impl<State: VerificationState> SignedMantleTx<State> {
    ...
    #[must_use]
    pub const fn ops_proofs(&self) -> &Vec<OpProof> {
        &self.ops_proofs
    }
...
impl SignedMantleTx<Unverified> {
    #[must_use]
    pub const fn new(mantle_tx: MantleTx, ops_proofs: Vec<OpProof>) -> Self {
        Self {
            mantle_tx,
            ops_proofs,
            state: PhantomData,
...
impl SignedMantleTx<Preverified> {
    ...
    #[must_use]
    #[doc(hidden)]
    pub const fn new_trusted(mantle_tx: MantleTx, ops_proofs: Vec<OpProof>) -> Self {
        Self {
            mantle_tx,
            ops_proofs,
            state: PhantomData,
...
mod tests {
    ...
    fn create_withdraw_tx(
        ...
        let tx = SignedMantleTx::new(mantle_tx, vec![OpProof::ChannelMultiSigProof(proof)])
            .preverify()
            .unwrap();
        assert_eq!(
    ...
    fn test_signed_mantle_tx_new_with_valid_inscribe_proof() {
        ...
        let result =
            SignedMantleTx::new(mantle_tx, vec![OpProof::Ed25519Sig(signature)]).preverify();
    ...
    fn test_signed_mantle_tx_new_missing_inscribe_proof() {
        ...
        let inscribe_op = create_test_inscribe_op(&signing_key);
        let mantle_tx = create_test_mantle_tx(vec![Op::ChannelInscribe(inscribe_op)]);
        let result = SignedMantleTx::new(mantle_tx, vec![]).preverify();
    ...
    fn test_signed_mantle_tx_new_incorrect_inscribe_proof_type() {
        ...
        let result = SignedMantleTx::new(mantle_tx, vec![zk_sig]).preverify();
    ...
    fn test_signed_mantle_tx_deserialize_with_valid_proof() {
        ...
        let signed_tx = SignedMantleTx::new(mantle_tx, vec![OpProof::Ed25519Sig(signature)])
            .preverify()
            .unwrap();
    ...
    fn test_signed_mantle_tx_deserialize_preverified_with_missing_proof() {
        ...
        let helper = SignedMantleTx::new(mantle_tx, vec![]);
    ...
    fn test_signed_mantle_tx_deserialize_preverified_with_invalid_signature() {
        ...
        let helper = SignedMantleTx::new(mantle_tx, vec![OpProof::Ed25519Sig(wrong_signature)]);
    ...
    fn test_signed_mantle_tx_new_proof_count_mismatch() {
        ...
        // Test too few proofs
        let result = SignedMantleTx::new(mantle_tx.clone(), vec![]).preverify();
        assert!(matches!(
            result,
            Err(VerificationError::ProofCountMismatch {
                ops_count: 1,
