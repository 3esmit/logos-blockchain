...
pub(crate) use crate::{
    ...
    run_commands::{
        ...
        types::{
            AuthorizedSigner, SignedWithdrawFile, WithdrawFileEntry, WithdrawIntent,
            WithdrawSignatureEntry, WithdrawSignatureFile,
        },
        utils::{
            decode_ed25519_public_key_hex, decode_hex_bincode, decode_mantle_tx_hex,
            decode_msg_id_hex, decode_note_id_hex, decode_signed_mantle_tx_hex,
...
pub(crate) async fn run_withdraw_prepare(args: WithdrawPrepareArgs) -> RunResult<()> {
    ...
    let note_ids = args
        .note_id
        .iter()
        .map(|note_id| decode_note_id_hex(note_id))
        .collect::<RunResult<Vec<_>>>()?;
    let withdraw = ChannelWithdrawOp {
        channel_id,
        inputs: Inputs::try_new(note_ids.clone())?,
    };
    ...
    let intent = WithdrawIntent {
        ...
        withdraws: note_ids
            .iter()
            .map(|note_id| WithdrawFileEntry {
                note_id: hex::encode(note_id.as_bytes()),
            })
            .collect(),
