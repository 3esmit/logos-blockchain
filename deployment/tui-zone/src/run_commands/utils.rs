...
use lb_core::mantle::{
    MantleTx, Note, NoteId, SignedMantleTx, TxHash, Utxo, Value,
...
/// Decode a hex-encoded note id.
pub fn decode_note_id_hex(value: &str) -> RunResult<NoteId> {
    let bytes = fixed_bytes::<32>(value)?;
    Ok(NoteId(lb_groth16::fr_from_bytes(&bytes)?))
}
