...
///
/// Each round, IBD fetches the chain tip from every trusted peer in
...
/// so block downloads themselves are not restricted to the trusted peer
/// set.
...
{
    /// Runs IBD against the trusted peers.
    ///
    /// # Returns
    ...
    pub async fn run(
        ...
        if config.trusted_peers.is_empty() {
            warn!("Skipping IBD as no trusted peers are configured");
            return Ok(self.block_processor);
        }

        info!(
            "Starting IBD with {} trusted peers",
            config.trusted_peers.len()
        );

        let mut downloader = OrphanBlocksDownloader::new(
            self.network.clone(),
            config.trusted_peers.len().try_into().unwrap(),
        );
    ...
    async fn drain_downloader(
        ...
        // download fails and leaves the queue empty: the downloader returns
        // `Pending` with no internal wake source. The timeout lets us re-check
        // `should_poll` and exit cleanly in that case.
            ...
            match tokio::time::timeout(Duration::from_secs(1), downloader.next()).await {
                Ok(Some(block)) => {
                    if let Err(e) = self.block_processor.process_block(block).await {
                        error!("Failed to process block during IBD: {e:?}");
...
/// of the peers that responded. Per-peer failures are warn-logged and dropped.
...
{
    join_all(peers.into_iter().map(async |peer| {
...
mod tests {
    ...
    #[tokio::test]
    async fn no_trusted_peers_configured() {
    ...
    /// If all trusted peers fail to return a tip, [`Error::AllPeersFailed`]
    /// is returned.
    #[tokio::test]
    ...
    async fn run_ibd(
        ...
        timeout(Duration::from_secs(5), ibd.run(config(peers)))
            .await
