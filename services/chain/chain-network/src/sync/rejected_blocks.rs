...
/// (known-invalid, older-than-LIB, etc.).
///
...
impl RejectedBlocks {
    ...
    /// Returns `true` if either the block itself or its parent is in the cache.
    /// Touches matching entries on hit so frequently-checked rejections stay
    /// in the cache.
    ///
    /// Always returns `false` when the cache is disabled.
    pub fn contains_block_or_parent(&mut self, block_id: &HeaderId, parent_id: &HeaderId) -> bool {
        let Some(cache) = self.cache.as_mut() else {
            return false;
        };
        cache.get(block_id).is_some() || cache.get(parent_id).is_some()
    }
