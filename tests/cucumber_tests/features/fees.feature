Feature: Fees
  ...
  # The gas price after every block must equal the spec's calculation,
  # recomputed here independently. Verifies the consensus math only; whether
  # the resulting prices are economically viable is covered by the
  # market-liveness unit tests in the ledger crate.
  @transactions_ci
  Scenario: Gas price updates match the fee market spec reference calculation
  ...
  # Storage usage must reach the pricing update: with traffic on chain, the
  # price must move at an epoch boundary. Guards that block application adds
  # each block's storage gas to the epoch consumption (which was missing
  # until now); the price viability problems remain covered by the red
  # ledger unit tests.
  @transactions_ci
  Scenario: Storage gas price responds to network usage
    Given the cluster uses cryptarchia security parameter 3
    And I have deployment config override "time.slot_duration" as "seconds(1)"
    And I have deployment config override "cryptarchia.slot_activation_coeff.numerator" as "1"
    And I have deployment config override "cryptarchia.slot_activation_coeff.denominator" as "2"
  ...
  # Concurrent transfers funded from the same wallet must spend disjoint
  # notes - otherwise one is a double-spend that never lands. Pins the
  # wallet's pending-note reservation.
  @transactions_ci
  Scenario: Concurrent wallet funded transfers are all included with disjoint inputs
    Given the genesis block has the following wallet resources:
      | account_index | token_count | token_amount |
      | 1             | 1           | 10000        |
      | 2             | 1           | 10000        |
    ...
    When node "NODE_1" is at height 2 in 240 seconds
    And I concurrently fund transfers of 500 LGO from wallets "FUNDER_A" and "FUNDER_B" to wallets "RECIPIENT_A" and "RECIPIENT_B" via node "NODE_1" as "TX_A" and "TX_B"
    ...
    And transactions "TX_A" and "TX_B" spent disjoint ledger inputs on node "NODE_1"
