...
use lb_core::mantle::{
    GasCalculator as _, Note, Op, SignedMantleTx, Transaction as _, Utxo,
    gas::MainnetGasConstants,
    transactions::{
        GasPrices, MantleTxBuilder, MantleTxContext, MantleTxGasContext,
        codec::encode_signed_mantle_tx,
    },
};
...
pub const SPEC_BASE_FEE_NUMERATOR: u128 = 11_176_760;
/// [1.0.0] Execution Market: `8 * G_target`.
pub const SPEC_BASE_FEE_DENOMINATOR: u128 = 12_773_440;
...
impl ExecutionMarketReference {
    ...
    pub fn apply_block(&mut self, block_execution_gas: u64) {
        ...
        self.base_fee =
            self.base_fee * (SPEC_BASE_FEE_NUMERATOR + self.gas_ema) / SPEC_BASE_FEE_DENOMINATOR;
    }
}
...
pub fn self_transfer_paying_fee_at(
    ...
    SignedMantleTx::new(mantle_tx, proofs).expect("signed transaction should be valid")
}
...
#[must_use]
pub fn fee_of(tx: &SignedMantleTx, prices: &GasPrices) -> u64 {
    let gas_context = MantleTxGasContext::new(HashMap::new(), HashMap::new(), prices.clone());
    tx.mantle_tx
...
pub fn check_size_prediction(tx: &SignedMantleTx, prices: &GasPrices) -> Result<(), String> {
    let gas_context = MantleTxGasContext::new(HashMap::new(), HashMap::new(), prices.clone());
    let predicted = tx.mantle_tx.signed_serialized_size(&gas_context);
...
/// genesis notes.
pub fn net_balance_against(genesis_utxos: &[Utxo], tx: &SignedMantleTx) -> Result<u64, String> {
...
#[must_use]
pub fn transfer_inputs(tx: &SignedMantleTx) -> Vec<lb_core::mantle::NoteId> {
    tx.mantle_tx
        .ops()
        .iter()
