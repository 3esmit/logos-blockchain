...
use lb_core::mantle::{
    NoteId, SignedMantleTx, Transaction as _, TxHash, Utxo,
...
impl ScannerAccounting {
        ...
        for op in tx.mantle_tx.ops() {
            match op {
                ...
                Op::ChannelDeposit(deposit) => {
                    ...
                    self.notes_to_channel(deposit.inputs.iter().copied(), deposit.channel_id);
                }
                Op::ChannelWithdraw(withdraw) => {
                    // A withdraw marks channel notes, as bedrock notes.
                    self.notes_to_bedrock(withdraw.inputs.iter().copied());
                }
    ...
    fn notes_to_channel(
        ...
        for note_id in note_ids {
            for utxos_by_note in self.wallet_utxos.values_mut() {
                if let Some(utxo) = utxos_by_note.get_mut(&note_id) {
    ...
    fn notes_to_bedrock(&mut self, note_ids: impl IntoIterator<Item = NoteId>) {
        for note_id in note_ids {
            for utxos_by_note in self.wallet_utxos.values_mut() {
                if let Some(utxo) = utxos_by_note.get_mut(&note_id) {
