...
use lb_core::{
    header::HeaderId,
    mantle::{NoteId, Transaction as _, TxHash, gas::GasPrice},
};
...
/// drives the price update at its boundary. Guards the block-application
/// side: each block's storage gas has to be added to the epoch consumption
/// for the update to ever see demand.
        ...
        return Err(StepError::StepFail {
            message: format!(
                "Step `{}` error: the storage gas price stayed at {first_price} for all {} \
                 ...
                 market is disconnected from usage (`storage_gas_consumed_in_epoch` is \
                 never incremented by block application)",
                step.value,
                records.len(),
            ),
        });
