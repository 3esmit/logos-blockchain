...
/// Parses an atomic-withdraw table row of `(alias, notes)` where `notes`
/// is a comma-separated list of note ids. Each row becomes one
...
pub(super) fn zone_atomic_withdraw_rows(step: &Step) -> Result<Vec<(String, usize)>, StepError> {
    parse_table_rows(
        ...
        |row| match row {
            ...
            _ => invalid_table_row("Atomic withdraw", &["withdraw", "notes"], row.len()),
        },
    )
}
