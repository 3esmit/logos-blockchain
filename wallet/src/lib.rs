...
use lb_core::{
    ...
    mantle::{
        ...
        ops::{
            Op, OpId as _,
            channel::{ChannelId, transfer::ChannelTransferOp},
...
pub enum WalletOp {
    ...
    Transfer(TransferOp),
    /// Spend the channel `inputs`, create the channel `outputs`.
    ChannelTransfer(ChannelTransferOp),
    /// Mark this note as locked.
    Lock(NoteId),
    /// Create the reward note.
    LeaderClaim(Utxo),
    /// Move the notes into the channel.
    ChannelDeposit(ChannelId, Vec<NoteId>),
    /// Move the notes back to the bedrock.
    ChannelWithdraw(Vec<NoteId>),
}
    ...
    pub fn spent_note_ids(&self) -> HashSet<NoteId> {
        self.txs
            ...
            .flat_map(|op| match op {
                ...
                WalletOp::ChannelTransfer(transfer) => {
                    transfer.inputs.iter().copied().collect::<Vec<_>>()
                }
                WalletOp::ChannelDeposit(_, inputs) => inputs.clone(),
...
impl WalletState {
    ...
    pub fn fund_tx<G: GasConstants>(
        ...
        // - Channel notes, which are only spendable through channel ops
    ...
    pub fn balance(&self, pk: ZkPublicKey) -> Option<WalletBalance> {
        ...
        self.pk_index.get(&pk)?.iter().for_each(|id| {
            let utxo = self.utxos[id];
            // Channel notes aren't spendable from the wallet.
        ...
        for tx in &block.txs {
            for op in &tx.ops {
                match op {
                    ...
                    WalletOp::ChannelDeposit(channel_id, inputs) => {
                        for input_id in inputs {
                            note_to_channel(input_id, *channel_id, &mut utxos);
                        }
                    }
                    ...
                    WalletOp::ChannelWithdraw(inputs) => {
                        for input_id in inputs {
                            note_to_bedrock(input_id, &mut utxos);
                        }
                    }
                }
            }
...
fn transform_op(op: &Op, event: Option<TxEventPayload>) -> Option<WalletOp> {
    match op {
        ...
        Op::ChannelTransfer(transfer) => Some(WalletOp::ChannelTransfer(transfer.clone())),
        ...
        Op::ChannelWithdraw(withdraw) => Some(WalletOp::ChannelWithdraw(
            withdraw.inputs.iter().copied().collect(),
        )),
...
// The note keeps its id and owner, so `pk_index` doesn't change.
fn note_to_channel(
    note_id: &NoteId,
    channel_id: ChannelId,
    utxos: &mut rpds::HashTrieMapSync<NoteId, Utxo>,
) {
    if let Some(utxo) = utxos.get_mut(note_id) {
        *utxo = utxo.to_channel(channel_id);
    }
}

// The note keeps its id and owner, so `pk_index` doesn't change.
fn note_to_bedrock(note_id: &NoteId, utxos: &mut rpds::HashTrieMapSync<NoteId, Utxo>) {
    if let Some(utxo) = utxos.get_mut(note_id) {
...
mod tests {
    ...
    use lb_core::{
        ...
        mantle::{
            ...
            ops::channel::{MsgId, inscribe::InscriptionOp},
    ...
    fn test_initialization() {
        ...
        let ledger = LedgerState::from_utxos(
            [
                Utxo::new_bedrock(tx_hash(0), 0, Note::new(100, alice)),
                Utxo::new_bedrock(tx_hash(0), 1, Note::new(20, bob)),
                Utxo::new_bedrock(tx_hash(0), 2, Note::new(4, alice)),
            ],
            &ledger_config(),
        );
    ...
    fn test_sync() {
        ...
        let block_3 = WalletBlock {
            ...
            txs: vec![WalletTx {
                ops: vec![
                    WalletOp::ChannelDeposit(
                        ChannelId::from([0; 32]),
                        vec![alice_80_nmo_utxo.id()],
                    ),
                    WalletOp::LeaderClaim(Utxo::new_bedrock(tx_hash(9), 0, Note::new(38, alice))),
                ],
            }],
        };
    ...
    #[test]
    fn test_channel_deposit_and_withdraw_keep_the_notes() {
        let alice = pk(1);
        let bob = pk(2);
        let genesis = HeaderId::from([0; 32]);
        let channel_id = ChannelId::from([0; 32]);
        let alice_utxo = Utxo::new_bedrock(tx_hash(1), 0, Note::new(42, alice));
        let bob_utxo = Utxo::new_bedrock(tx_hash(1), 1, Note::new(7, bob));
        let ledger = LedgerState::from_utxos([alice_utxo, bob_utxo], &ledger_config());
        ...
        // The deposit moves the notes into the channel and the withdraw moves
        // them back, both keeping their id, so alice still owns hers.
        let block = WalletBlock {
            id: HeaderId::from([1; 32]),
            parent: genesis,
            epoch: 1.into(),
            ...
            txs: vec![WalletTx {
                ops: vec![
                    WalletOp::ChannelDeposit(channel_id, vec![alice_utxo.id(), bob_utxo.id()]),
                    WalletOp::ChannelWithdraw(vec![alice_utxo.id(), bob_utxo.id()]),
                ],
            }],
        };
    ...
    fn test_sdp_reward_credits_owned_provider() {
        ...
        let alice_reward = Utxo::new_bedrock(tx_hash(1), 0, Note::new(100, alice));
        let bob_reward = Utxo::new_bedrock(tx_hash(1), 1, Note::new(50, bob));
    ...
    fn test_fund_tx_with_change() {
        let alice = pk(1);
        let utxo1 = Utxo::new_bedrock(tx_hash(0), 0, Note::new(5000, alice));
        let utxo2 = Utxo::new_bedrock(tx_hash(0), 1, Note::new(5000, alice));
    ...
    fn test_fund_tx_with_priority_fee() {
        let alice = pk(1);
        let utxo = Utxo::new_bedrock(tx_hash(0), 0, Note::new(5000, alice));
    ...
    fn test_fund_tx_insufficient_funds() {
        ...
        let ledger_state = LedgerState::from_utxos(
            [
                Utxo::new_bedrock(tx_hash(0), 0, Note::new(100, alice)),
                Utxo::new_bedrock(tx_hash(0), 1, Note::new(100, alice)),
                Utxo::new_bedrock(tx_hash(0), 2, Note::new(100, alice)),
                Utxo::new_bedrock(tx_hash(0), 3, Note::new(100, alice)),
            ],
            &ledger_config(),
        );
    ...
    fn test_fund_tx_respects_pk_list() {
        ...
        let bob = pk(2);
        let ledger_state = LedgerState::from_utxos(
            [Utxo::new_bedrock(tx_hash(0), 0, Note::new(1_000_000, bob))],
            &ledger_config(),
        );
    ...
    fn test_fund_tx_unfundable_region() {
        ...
        assert_eq!(
            ...
            tx_builder
                .clone()
                .add_ledger_input(Utxo::new_bedrock(tx_hash(0), 0, Note::new(0, pk(0))))
                .unwrap()
                .gas_cost::<Gas>(&context)
                .unwrap()
                .into_inner()
        ...
        let wallet_state = WalletState::from_ledger(
            ...
            &LedgerState::from_utxos(
                [Utxo::new_bedrock(tx_hash(0), 0, Note::new(754, alice))],
                &ledger_config(),
            ),
        );
        ...
        for value in 755..=794 {
            ...
            let wallet_state = WalletState::from_ledger(
                &HashMap::from_iter([(alice, 1)]),
                &LedgerState::from_utxos(
                    [Utxo::new_bedrock(tx_hash(0), 0, Note::new(value, alice))],
                    &ledger_config(),
                ),
            );
        ...
        let wallet_state = WalletState::from_ledger(
            &HashMap::from_iter([(alice, 1)]),
            &LedgerState::from_utxos(
                [Utxo::new_bedrock(tx_hash(0), 0, Note::new(795, alice))],
                &ledger_config(),
            ),
        );
